...
contract LPCertNFT is ERC721, Ownable {
    error LPCertNFT__InvalidMilestone(string milestone);
    error LPCertNFT__MaximumQuantityReached(string milestone, uint16 maxSupply);
    error LPCertNFT__OnlyOwnerOrCollateralOwner(address _opeartor);

    event LPCertNFT__NFTMinted(
        uint256 indexed tokenId,
        address indexed owner,
        string indexed milestone,
        uint16 holderRank
    );

    uint256 private s_tokenCounter;
    uint16 private s_defaultMaxSupply;
    string[] private s_milestones;
    ...
    function mintNft(
        ...
        string[] memory countries = s_milestones;
        if (!isValidCountry(_countryName, countries)) {
            revert LPCertNFT__InvalidMilestone(_countryName);
        }
        ...
        if (
            ...
            revert LPCertNFT__MaximumQuantityReached(
                _countryName,
    ...
    function setCountriesMaxSupply(
        ...
        if (_countries.length > s_milestones.length) {
            revert LPCertNFT__InvalidMilestone(
        ...
        for (uint256 i = 0; i < _countries.length; i++) {
            if (!isValidCountry(_countries[i], countries)) {
                revert LPCertNFT__InvalidMilestone(_countries[i]);
            }
    ...
    modifier OnlyOwnerOrCollaborators() {
        if (msg.sender != owner() && !s_collateralOwners[msg.sender]) {
            revert LPCertNFT__OnlyOwnerOrCollateralOwner(msg.sender);
        }
        _;
    }
